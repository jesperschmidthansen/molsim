...
void sep_read_angles_top(sepatom *aptr, sepmolinfo *ptr, const char *file, int npart, char opt){
	...
	do {
		...
		if ( line[0] == '[' ){ 
			...
			if ( aindex[a] > SEP_ANGLE-1 || aindex[a] > SEP_ANGLE-1 || aindex[a] > SEP_ANGLE-1 )
				sep_error("%s at line %d: Index exceeds SEP_ANGLE", __func__, __LINE__);

			aptr[a].angle[aindex[a]] = b; aindex[a]++; aptr[a].angle[aindex[a]] = c; aindex[a]++;
			aptr[b].angle[aindex[b]] = a; aindex[b]++; aptr[b].angle[aindex[b]] = c; aindex[b]++;
			aptr[c].angle[aindex[c]] = a; aindex[c]++; aptr[c].angle[aindex[c]] = b; aindex[c]++;
