...
// Represents a molecule defined via the xyz and top files
typedef struct {
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
	...
	if ( nlhs > 0 || nrhs != 5 ){
		mexErrMsgTxt("Input error for molconfig");
	...
	char *xyzfile1 = mxArrayToString(prhs[0]);      
	char *topfile1 = mxArrayToString(prhs[1]);
	char nmol1[32]; sprintf(nmol1, "%d",(int)mxGetScalar(prhs[2]));
	char dens[32]; sprintf(dens, "%.6f", mxGetScalar(prhs[3]));
	char seed[32]; sprintf(seed, "%d", (int)mxGetScalar(prhs[4]));
...
void sep_sfg(int argc, char **argv){
	...
	writexyzheader(natoms, lat.lbox, "conf.xyz");
	writebondheader("bonds.top");
	writeangleheader("angles.top");
	writedihedralheader("dihedrals.top");

	// Write the config files
	for ( int n=0; n<nmol_tot; n++ ){
		writexyz(mols, rlist[n], lat, n, lat.lbox, seed, "conf.xyz");
		writebondssect(mols, rlist[n], n, "bonds.top");
		writeanglesect(mols, rlist[n], n, "angles.top");
		writedihedralsect(mols, rlist[n], n, "dihedrals.top");
	}
	//printf("Done\n");
...
void printpre(int nmol_tot, int ntypes ){
	...
	printf("This program is a part of the sep library (now molsim)- there is absolutely ");
	printf("NO WARRANTY\n\n");
